<template>
    <div id="tutors">
        ...
        <div id="content" v-on:click="updateFocus('', $event)">
            ...
            <strong>
                Die Tutor*innen sind Studierende, die euch beim Einstieg ins
                Studium begleiten und euch zur Seite stehen, eine Menge zeigen
                und die Kneipen um die FH mit euch unsicher machen.
                <br /><br />
                Aus den Untiefen des Heimstudiums erreichte uns folgende
                Übertragung.
            </strong>
            <p>
                01010100011101010111010001101111011100100110010101101110
                <br /><br />
                Aufgepasst Erstis! Mit Beginn eures Studiums an der FH Dortmund
                seid ihr eurem Ziel ein erfolgreicher Designer zu werden schon
                einen Schritt näher - woher wir das wissen? Wir sind die
                Ersti-tutoren aus der Zukunft und wurden ins Jahr 2020 entsandt,
                um euch während eures ersten Semesters zur Seite zu stehen.
                <br /><br />
                Auf euch wartet eine spannende Reise voller neuer Aufgaben,
                unvergesslicher Teamarbeiten, innovativen Designs, legendären
                Kneipentouren, sagenumwobenen Feten und immerwährenden
                Freundschaften. Coronabedingt saßen wir alle lang genug alleine
                rum, deshalb wird es Zeit in Zukunft wieder gemeinsam den
                Dancefloor unsicher zu machen.Solange das analog nicht möglich
                ist, könnt ihr euch digital mit uns die Zeit vertreiben: mit
                Podcasts rund ums Studium, Onlinespielen und Livestreams von
                euren Future Tuts.
            </p>
            <Calendar />
            <p>
                Die Zukunft. Hier feiern die fetzigsten Dinge der Vergangenheit
                ihr Comeback: Neonlicht, Aluhüte und LED-Heelys dürfen auf
                keiner Future Party fehlen. Sounds like fun? Dann sucht euch ein
                grooviges Partyoutfit aus der Zukunft zusammen, schwingt euch
                aufs Hoverboard und lasst uns mit den Leuchtstoffröhren um die
                Wette strahlen!
                <br /><br />
                01010100011101010111010001101111011100100110010101101110
            </p>
    ...
    mounted() {
        this.observer = new MutationObserver(mutations => {
            for (const m of mutations) {
                ...
                this.$nextTick(() => {
                    this.expanded =
                        newValue.contains("expanded") ||
                        newValue.contains("focused");
                });
            }
        });
            ...
            if (!this.expanded) {
                x =
                    Math.cos(this.primeAngle * index * (Math.PI / 180)) *
                        this.primeRadiusX *
                        (Math.floor((this.primeAngle * (index + 1)) / 360) +
                            1) +
                    "%";
                y =
